...
export function createProviderConfig(
  ...
  return {
    ...
    responsesPromptRevisionMode,
    id,
    name: name.trim() || '未命名GPT-Image2',
  }
}

export function getNextProviderName(providers: ProviderConfig[]): string {
  let index = providers.length + 1
  while (providers.some((provider) => provider.name === `GPT-Image2 ${index}`)) {
    index += 1
  }
  return `GPT-Image2 ${index}`
}
  ...
  return providers
    .map((provider, index) => {
      ...
      return createProviderConfig(
        ...
        typeof name === 'string' ? name : `GPT-Image2 ${index + 1}`,
...
export function remapImportedTaskRelations(
  ...
  return {
    ...
    providerName: mappedProvider?.name?.trim() || task.providerName || '未知GPT-Image2',
